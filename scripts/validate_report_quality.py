#!/usr/bin/env python3
"""Validate a paper reading report before building or publishing a Notion page."""
...
def validate_structure(text: str, language: str, notion_body: bool, errors: list[str], warnings: list[str]) -> None:
    if not notion_body and not text.lstrip().startswith("# "):
...
def validate_images(
    ...
    if html_figures and image_status != "hosted":
        errors.append("reachable arXiv HTML figures exist, but image_status is not hosted")

    if html_figures and not links:
        errors.append("reachable arXiv HTML figures exist, but report embeds no hosted images")
    if html_figures and links:
    ...
    if image_status == "local_only":
        if not re.search(r"evidence_pack\.html|local evidence pack|local-only|local only", text, re.IGNORECASE):
            warnings.append("local_only report should point to an evidence pack or local-image note")


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("report", type=Path, help="Markdown report to validate")
    parser.add_argument("--payload", type=Path, help="Optional Notion payload for language, paper type, and image status")
    ...
    parser.add_argument("--notion-body", action="store_true", help="Allow a Notion page body without a top-level H1 title")
    ...
    validate_structure(text, language, args.notion_body, errors, warnings)
