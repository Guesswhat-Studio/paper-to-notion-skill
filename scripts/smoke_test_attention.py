...
This smoke-test report confirms that the local environment can download a paper PDF, open it with PyMuPDF, extract first-page text, render a header image, and build a Notion payload.
...
This is a setup validation artifact. If Notion write access is available, it can be published as a `[TEST] Attention Is All You Need` page and removed or archived after validation.
"""
...
def run_payload_build(output_dir: Path) -> None:
    root = skill_root()
    payload_path = output_dir / "notion_payload.json"
    build_script = root / "scripts" / "build_notion_payload.py"
    validate_script = root / "scripts" / "validate_notion_payload.py"
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".paper-notion/smoke-test"))
    ...
    print("Building and validating Notion payload")
