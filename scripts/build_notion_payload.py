#!/usr/bin/env python3
"""Build a Notion paper payload from metadata and an optional report."""
...
def convert_inline_math_line(line: str) -> str:
    ...
    while index < len(line):
        ...
        if char == "`":
            ...
            index = end + 1
            continue
        if char != "$" or (index > 0 and line[index - 1] == "\\"):
            result.append(char)
            index += 1
            continue
        ...
        end = find_unescaped_dollar(line, index + 1)
        if end == -1:
            result.append(char)
            index += 1
            continue

        expression = line[index + 1 : end]
        if expression.strip():
...
def convert_inline_math_to_notion(markdown: str) -> str:
    lines: list[str] = []
    in_code = False
    ...
    for line in markdown.splitlines(keepends=True):
        ...
        if in_code or in_display_math:
            ...
            lines.append(convert_inline_math_line(body) + newline)
    return "".join(lines)
...
def dedup_key(doi: str, arxiv: str, original_title: str) -> tuple[str, str]:
    ...
    if arxiv:
        ...
        return (match.group(1).lower() if match else arxiv.lower()), "arxiv"
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, help=".paper-notion/config.json path")
    parser.add_argument("--schema", type=Path, default=DEFAULT_SCHEMA, help="Notion schema YAML path")
    parser.add_argument("--output", type=Path, default=Path("notion_payload.json"))
    parser.add_argument("--language", choices=sorted(LANGUAGES), default="English")
    parser.add_argument("--image-status", choices=["hosted", "local_only", "placeholder", "no_images"], default="placeholder")
    parser.add_argument("--math-format", choices=["notion", "markdown"], default="notion")
    ...
    schema_props = schema_property_map(load_schema(args.schema))
    ...
    if args.report:
        ...
        if args.report.exists():
            ...
            if args.math_format == "notion":
                report_markdown = convert_inline_math_to_notion(report_markdown)

    payload = {
        "database": {
            "database_id": str(config.get("notion_database_id", "")),
            "data_source_id": str(config.get("notion_data_source_id", "")),
        },
        ...
        "properties": {
            ...
            "Type": paper_type,
            "Keywords": keyword_text(first_value(metadata, "Keywords", "keywords")),
        ...
        "content": {
            ...
            "notion_markdown": report_markdown,
        },
    }
