...
When publishing through a Notion connector that cannot upload local files, keep the extracted images in the local `images/` folder and build a self-contained local `evidence_pack.html`. Write a short image-status note in the Notion page that points to that single local evidence pack. Do not pretend that local images are embedded. Do not use external image hosting unless the user explicitly allows it.
...
- Reader Rating: Include a 1 to 5 star rating only if the user provides it or asks the agent to judge. Explain the reason in the page body, not the database.
